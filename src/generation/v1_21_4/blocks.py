        ...
        with open(
            ...
            for block in self.blocks:
                ...
                with open(
                    ...
                    file2.write(
                        f'{self.header}execute as @e[type=item,sort=nearest,limit=1,distance=..2,nbt={{OnGround:0b,Age:0s,Item:{{id:"minecraft:{self.blocks[block]["baseBlock"]}"}}}}] run kill @s\n'
            ...
            with open(
                ...
                if self.blocks[block]["blockDrop"] == "self":
                    ...
                    if self.blocks[block]["blockDrop"] not in self.items and self.blocks[block]["blockDrop"] not in self.blocks:
                        ...
                        file.write(
                            f'{{"pools": [{{"rolls": 1,"entries": [{{"type": "minecraft:item","name": "{item["baseItem"]}","functions":[{{"function": "minecraft:set_components","components":{{"minecraft:item_name": "{{\\"italic\\":false,\\"text\\":\\"{item["displayName"]}\\"}}","minecraft:item_model": "{self.packNamespace}:{item["name"]}"}}}}]}}]}}]}}'
                        )
